var tableClasses = $("#tests-table").DataTable({
...
function del(url){
    bootbox.confirm({
        message: "Voulez-vous vraiment supprimer ce test ? <br> Cela supprimera toutes les réponses, bilans etc... des étudiants liés à ce test <br><br> Cette action est irréversible.",
        buttons: {
            confirm: {
                label: 'Oui',
    ...
    $.ajax({
        ...
        success: function(json, statut){
            if(!json.result.error){
                ...
                tableClasses
                    .row($("#test_" + json.result.id).parent())
                    .remove()
                    .draw();
            }else{

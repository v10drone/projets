...
var tableTests = $("#tests-table").DataTable({
    "sPaginationType": "full_numbers",
    aoColumnDefs: [
        {
            bSortable: false,
...
function del(url){
    bootbox.confirm({
        message: "Voulez-vous vraiment supprimer cet étudiant de ce groupe ? <br> Cette action est irréversible.",
        buttons: {
            confirm: {
                label: 'Oui',
    ...
    $.ajax({
        ...
        success: function(json, statut){
            if(!json.result.error){
                ...
                tableStudents
                    .row($("#student_" + json.result.id).parent())
                    .remove()
                    .draw();
            }else{

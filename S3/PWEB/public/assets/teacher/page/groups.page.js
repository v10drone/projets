var tableClasses = $("#classes-table").DataTable({
...
function del(url){
    bootbox.confirm({
        message: "Voulez-vous vraiment supprimer ce groupe ? <br> Cette action est irréversible.",
        buttons: {
            confirm: {
                label: 'Oui',
    ...
    $.ajax({
        ...
        success: function(json, statut){
            if(!json.result.error){
                ...
                tableClasses
                    .row($("#group_" + json.result.id).parent())
                    .remove()
                    .draw();
            }else{
